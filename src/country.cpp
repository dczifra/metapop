...
void Country::init_agents(std::map<int,int>& inf_cities){
    ...
    while(act_city < graph.cityNum){
        if(act_pop == graph.population[act_city]){
            ...
            if(infect-- > 0){
                agents[states][i] = I;
                agents[timers][i] = std::min(10,I_time(generator)+1);
                //agents[timers][i] = 10;
            }
...
void Country::infection(std::array<int, SEIR_SIZE>& stats){
    ...
    for(unsigned int city=0;city<graph.cityNum;city++){
        ...
        if(S_city == 0){
            ...
            long double p_simple = std::pow((1-contactNum*args.beta/pop), I_city);
...
void init_stats(Graph& graph, std::array<int,SEIR_SIZE>& base){
    for(unsigned int city=0;city<graph.cityNum;city++){
...
void Country::simulate(){
    ...
    for(int iteration_num=1;iteration_num<=args.max_sim; iteration_num++){
        ...
        // 2. Infect agents
        infection(stats[iteration_num]);
        std::cout<<stats[iteration_num];
        //debug(iteration_num, agents);
